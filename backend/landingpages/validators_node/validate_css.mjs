...
// script path. Never reads .stylelintrc / stylelint.config.js / any
// package-level config; never accepts a config path, plugin path, parser
// path, or JavaScript callback from its input. Reads exactly one JSON
// object from stdin, writes exactly one JSON document to stdout. Never
// executes or evaluates the submitted CSS, never makes a network request.
...
const ENGINE_LABEL = 'postcss-stylelint-csstree';
const ENGINE_VERSION = `postcss@${POSTCSS_VERSION};stylelint@${STYLELINT_VERSION};css-tree@${CSSTREE_VERSION}`;
...
const KNOWN_CONTEXTS = new Set(['stylesheet', 'declaration-list']);
...
function buildStylelintConfig(context) {
  const rules = {
    'block-no-empty': true,
...
function excerptFor(sourceLines, line) {
...
function checkStructuralBalance(css) {
  const issues = [];
  const openStack = [];
  let line = 1;
  let column = 1;
...
// check, and runLegacyPipeline below for the pre-existing PostCSS/Stylelint
// pipeline, kept independent so a hard failure in either never removes the
// other's results).
//
...
function runStructuralAndSemanticAnalysis(css) {
  const issues = [];
  const parseErrors = [];
...
async function main() {
  ...
  if (!KNOWN_PROFILES.has(profile)) {
    writeResult(buildFailure('CSS_PROFILE_INVALID', `Unknown validation profile: ${JSON.stringify(profile)}.`, startedAt));
    return;
  }
  ...
  // Three independent phases, each guarded so a hard failure in one never
  // removes another's findings (Sprint CSS-A-2 requirement 4: never stop
  // after the first parser failure).
  //
  // 1. Pure lexical brace/string/comment balance — always runs, never
  //    throws, needs no CSS grammar knowledge.
  const issues = checkStructuralBalance(css);

  // 2. css-tree's recovering parser + standards-derived property/value
  //    lexer — structural and semantic diagnostics, continuing past most
  //    defects instead of aborting the whole document.
  const structural = runStructuralAndSemanticAnalysis(css);
  issues.push(...structural.issues);

  // 3. The pre-existing PostCSS/Stylelint/custom-checks pipeline — still
  //    the source of style-convention findings (duplicate declarations,
  //    the security/responsive/accessibility custom checks, etc.).
  //    PostCSS's own parser is NOT error-recovering: one hard syntax
  //    error aborts its whole parse. That is no longer treated as "the
  //    document has exactly one problem" (phases 1–2 already cover that
  //    class of defect with real recovery) — it just means this
  //    particular phase contributes nothing for this document, silently,
  //    rather than emitting a single misleading generic message that
  //    used to overwrite everything else.
  let parseError = null;
  try {
    const root = postcss.parse(css);

    const stylelintResult = await stylelint.lint({ code: css, config: buildStylelintConfig(context) });
    ...
    if (err && err.name === 'CssSyntaxError') {
      // Recorded only as a signal for embedded-CSS batching (see
      // node_bridge.py/embedded_css.py) — phases 1–2 above already
      // produced the real diagnostics for this document.
      parseError = err;
    } else {
      writeResult(buildFailure('CSS_ENGINE_FAILED', 'CSS validation could not be completed.', startedAt));
      return;
    }
  }

  const truncated = issues.length > maxIssues;
  const limited = issues.slice(0, maxIssues).map((issue) => ({
    ...issue,
    codeExcerpt: excerptFor(sourceLines, issue.line),
  }));
